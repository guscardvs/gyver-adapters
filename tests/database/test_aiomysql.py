...
from gyver.adapters.database.aiomysql import ConnectionProxy
from gyver.adapters.database.aiomysql import AiomysqlAdapter
...
async def adapter(factory: AdapterConfigFactory):
    ...
    adapter = AiomysqlAdapter(config)
    await adapter.init()
...
@pytest.fixture
async def connection(adapter: AiomysqlAdapter):
    context = adapter.context()
...
async def test_adapter_initialized(adapter: AiomysqlAdapter):
...
async def test_close_connection(adapter: AiomysqlAdapter):

...
from gyver.adapters.database.asyncpg import ConnectionProxy
from gyver.adapters.database.asyncpg import AsyncpgAdapter
...
async def adapter(factory: AdapterConfigFactory):
    config = factory.load(DatabaseConfig, "pg")
    adapter = AsyncpgAdapter(config)
    await adapter.init()
...
@pytest.fixture
async def connection(adapter: AsyncpgAdapter):
    context = adapter.context()
...
async def createmany(connection: ConnectionProxy, n: int):
    ...
    for _ in range(n):
        await connection.execute(
            "INSERT INTO my_table (name) VALUES ($1)", "John"
        )
    await connection.commit()


async def test_adapter_initialized(adapter: AsyncpgAdapter):
...
async def test_close_connection(adapter: AsyncpgAdapter):
...
async def test_transaction(connection: ConnectionProxy):
    await connection.transaction()
    await connection.execute(
        "INSERT INTO my_table (id, name) VALUES ($1, $2)", 11, "John"
    )
    await connection.commit()

    # Verify the data was inserted correctly
    rows = await connection.fetch("SELECT * FROM my_table WHERE id = $1", 11)
    assert len(rows) == 1
    assert rows[0]["id"] == 11
...
async def test_rollback(connection: ConnectionProxy):
    await connection.transaction()
    await connection.execute("INSERT INTO my_table (name) VALUES ($1)", "John")
